...
支持项目级配置：通过.zaivim/zai_project.yaml文件配置Docker容器参数
"""
...
class TaskboxExecutor:
    ...
    支持从.zaivim/zai_project.yaml加载项目级配置
    """
...
def invoke_shell_contained_execute(
    command: str,
    timeout: int = DEFAULT_TIMEOUT,
    working_dir: str = None,
    enable_network: bool = True,
...
#   }
# },
def invoke_shell_contained_info() -> Dict[str, Any]:
    """获取沙盒环境信息"""
    try:
...
#   }
# }
def invoke_shell_contained_cleanup():
    """清理持久化容器（如果存在）"""
    try:
        executor = _get_executor()
...
#            result = invoke_shell_contained_info()
#            print(json.dumps(result, indent=2))
#    else:
