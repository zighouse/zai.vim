...
- `:compact [instructions]` - 将对话上下文压缩为结构化摘要
- `:compact auto` - 切换自动压缩模式（token 超限时自动压缩）
- `:resume [session-id]` - 从 JSONL 持久化文件恢复之前的会话
- `:sessions` - 列出所有已保存的会话
...
Zai.Vim 使用统一的配置目录 `~/.zaivim/` 存储所有配置文件。

从当前工作目录向上搜索项目配置文件：
- `.zaivim/project.yaml`（新格式 - 推荐）
- `zai.project/zai_project.yaml`（旧格式 - 向后兼容）

### 配置文件说明

| 文件 | 用途 | 优先级 |
|------|------|-------|
| `~/.zaivim/assistants.yaml` | 用户配置（providers、API keys） | 1（用户全局） |
| `.zaivim/project.yaml` | 项目配置（sandbox、工作目录） | 2（项目特定） |
| `~/.zaivim/engine.pid` | 引擎进程 ID 文件 | 运行时 |

### Node.js 版本配置结构

对于 Node.js 版本（`@zaivim/engine`），配置格式与 Python 版本不同：

**用户配置** (`~/.zaivim/assistants.yaml`):
```yaml
services:
  openai:
...
详细的配置规范和迁移指南，请参见：
- **决策记录**: `docs/adr-config-naming.md` - 配置命名规范决策
- **迁移指南**: `docs/config-migration-guide.md` - 从 Python 版本迁移到 Node.js 版本

### Python 版本配置（旧系统）

对于 Python 版本（`python3/aichat.py`），使用 `zai.project/zai_project.yaml` 配置文件。上方示例展示了 Python 版本的配置结构。
