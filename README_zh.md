...
退出 nvim 之后再次加载时，lazy.nvim 会自动进行安装。可以使用 lazy.nvim 进行更新。

手动安装:

```
cd ~/.vim/plugged
git clone https://github.com/zighouse/zai.vim
```
...
Zai 会妥善保存和 DeepSeek 大语言模型服务所做的每一次交互。 每次交互完成时，都会在交互信息展示窗口中明确打印出当前的 Zai 会话内容自动保存的日志文件路径。所以不必担心之前做过的交互会丢失。

从打开 Zai 界面到使用命令 `:ZaiClose` 关闭 Zai 界面或者使用 `:exit`/`:quit` 会话指令结束当前 Zai 会话任务为止，构成一个 Zai 会话。针对每个 Zai 会话都会保存一个日志文件。默认情况下，Zai 会话的日志文件在 Linux/Mac 中会存放在 `~/.local/share/zai/log` 路径下，在 Windows 中会存放在  `User\AppData\Local\zai\log` 路径下。

### 关闭窗口与继续交互

在 Vim 中您可以从 Zai 的输入框或者交互信息展示窗口中通过 `:q` 命令直接关闭 Zai 的界面，这样做并不会丢失 Zai 完成的交互记录，也不会结束当前正在进行的 Zai 的后台交互任务。您可以再次打开 Zai 界面来阅读交互信息展示窗口中的内容或者后台任务的结果，也可以继续在输入框中进行更加深入的交互。

注意事项：
1. 在输入框中使用 Zai 的会话指令 `:exit`/`:quit` 会结束当前会话但保留窗口
2. 在 Vim 命令模式下使用`:ZaiClose`会关闭窗口并结束会话
...
由于 DeepSeek 大语言模型的服务是无状态服务，所以当用户需要多次逐层深入探索时，需要使用链式模式来带上历史记录作为新谈话的上下文，但这样也会使得后面的交互所使用的 tokens 积累增加。上下文积累 tokens 的长度不仅会导致费用的增加，还会导致新谈话中有效 tokens 的长度变短。而使用即时模式就不一样了。对于大语言模型来说，每次交互都是一次全新的开始，所以并不会产生链式模式下的问题。只是当需要在即时模式下想逐一深入探求一个复杂问题时，需要用户把之前已经获得的重要结论也在新的询问中带进来。
...
需要注意的，这里的会话指令是在 Zai 输入框中输入的内容，而不是在 Vim 命令模式下输入的 Vim 命令。

在 Zai 输入框中，您随时可以使用以下前缀为 `:` 的特殊指令来改变接下来与大语言模型交谈时的模型、提示词或者参数等：

- `:help` - 显示帮助信息
- `:exit`/`:quit` - 结束当前的 Zai 会话 (再次交互将是新会话)
...
在输入框中输入如下指令，并使用 `:ZaiGo`/`<Leader>zg` 发送即在信息展示窗口中显示 Zai 会话指令的详细信息。

### 会话模式

链式模式下，会将历史会话上下文与最新请求内容一并发送。在输入框中包含如下指令，并使用 `:ZaiGo`/`<Leader>zg` 发送即设置成了链式模式：

```
:talk_mode chain
```

即时模式下，仅发送最新请求内容。在输入框中包含如下指令，并使用 `:ZaiGo`/`<Leader>zg` 发送即设置成了即时模式：

```
:talk_mode instant
```

无论选择何种模式，所有会话内容都会记录到同一日志文件。如需新建会话，请使用 `:ZaiClose`/`<Leader>zX` 关闭当前会话以及 Zai 界面。也可以在不关闭 Zai 界面的情况下，在输入框中发送 `:exit`/`:quit` 指令。
...
可以在指令前缀和指令名之间插入一个减号 `-` 的指令形式来清除之前所做的参数设置。例如，清除之前设置的创造性参数 `temperature` 的设置，使用如下指定：
...
致敬 Vim，在 Zai 中默认使用冒号 `:` 作为会话指令的前缀。
